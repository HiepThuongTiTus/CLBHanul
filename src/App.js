// import logo from './logo.svg';
// import './App.css';

// function App() {
//   return (
//     <div>
//       <header>
//       <section class="nav-title">
//             <div class="item t0">
//               <i class="fa fa-map-marker" aria-hidden="true"></i> 08 Nguyễn Chánh - Đà Nẵng 
//             </div>
//             <div class="item t1">
//                 <i class="fa fa-phone t"></i> 0961547617
//             </div>
//         </section>
//       <section>  
//         <img class="titleanh" src="7.jpg" alt="Hiep"/>
//         <div class="topnav" id="myTopnav">
//           <a href="#home" class="tile">HOME</a>
//           <a href="#news">NEWS</a>
//           <a href="#activate">ACTIVITY</a>
//           <a href="#about">ABOUT</a>
//           <a href="#login">REGISTER</a>
//           <a href="#">
//             <form class="form-inline">
//               <input class="form-control mr-sm-2" type="search" placeholder="Search" aria-label="Search"/>
//               <button class="btn btn-outline-success my-2 my-sm-0" type="submit">Search</button>
//             </form> 
//           </a>
//           <a href="javascript:void(0);" class="icon" onclick="myFunction()">
//             <i class="fa fa-bars"></i>
//           </a>
//         </div>
//         <div class="container" >
//           <div class="row">
//               <div class="col-lg-9">
//                 <h3 class="titlechu">TAEKWONDO HANUL</h3>
//               </div>
//               <div class="col-lg-3 d-flex align-items-center justify-content-center" >
//                   <a href="https://www.facebook.com/plugins/video.php?height=317&href=https%3A%2F%2Fwww.facebook.com%2Fhanuldanangclub%2Fvideos%2F766767617306932%2F&show_text=false&width=560&t=0" class="venobox play-btn" data-vbtype="video" data-autoplay="true"></a>
//               </div>
//           </div>
//         </div>
//         <div class="polaroid rotate_right">
//           <img src="1.jpg" alt="Pulpit rock" width="100" height="120"/>
//           <p class="caption">Talent</p>
//         </div>

//         <div class="polaroid rotate_left">
//           <img src="2.jpg" alt="Monterosso al Mare" width="100" height="80"/>
//           <p class="caption">Hanul Taekwondo</p>
//         </div>

//         <p class="titles">
//           Chào mừng bạn đến với CLB Hanul Taekwondo của chúng tôi!
//         </p>
//       </section>
//       <section>
//         <h3 class="textt">ACTIVITY</h3>
//         <div class="container">
//             <div class="row">
//                 <div class="col-md-3 cardd">
//                       <img class= "bo" src="1.jpg" alt="Card image"  /> 
//                       <div class="card-img-overlay">
//                         <p class="card-text cardd1">08 Nguyễn Chánh - Đà Nẵng</p>
//                       </div>
//                 </div>
//                 <div class="col-md-3 cardd">
//                       <img class= "bo" src="2.jpg" alt="Card image" />
//                       <div class="card-img-overlay">
//                         <p class="card-text cardd1">08 Nguyễn Chánh - Đà Nẵng</p>
//                       </div>
//                 </div>
//                 <div class="col-md-3 cardd">
//                       <img class= "bo" src="3.jpg" alt="Card image" />
//                       <div class="card-img-overlay">
//                         <p class="card-text cardd1">08 Nguyễn Chánh - Đà Nẵng</p>
//                       </div>
//                 </div>
//                 <div class="col-md-3 cardd">
//                       <img class= "bo" src="4.jpg" alt="Card image"/>
//                       <div class="card-img-overlay">
//                         <p class="card-text cardd1">08 Nguyễn Chánh - Đà Nẵng</p>
//                       </div>
//                 </div>
//                 <div class="col-md-3 cardd">
//                       <img class= "bo" src="5.jpg" alt="Card image" />
//                       <div class="card-img-overlay">
//                         <p class="card-text cardd1">08 Nguyễn Chánh - Đà Nẵng</p>
//                       </div>
//                 </div>
//                 <div class="col-md-3 cardd">
//                       <img class= "bo" src="6.jpg" alt="Card image"  />
//                       <div class="card-img-overlay">
//                         <p class="card-text cardd1">08 Nguyễn Chánh - Đà Nẵng</p>
//                       </div>
//                 </div>
//                 <div class="col-md-3 cardd">
//                       <img class= "bo" src="13.jpg" alt="Card image"/>
//                       <div class="card-img-overlay">
//                         <p class="card-text cardd1">08 Nguyễn Chánh - Đà Nẵng</p>
//                       </div>
//                 </div>
//                 <div class="col-md-3 cardd">
//                       <img class= "bo" src="14.jpg" alt="Card image" />
//                       <div class="card-img-overlay">
//                         <p class="card-text cardd1">08 Nguyễn Chánh - Đà Nẵng</p>
//                       </div>
//                 </div>
//             </div>
//         </div>
//       </section>
//      <section>
//         <div class="container">
//           <div class="row">
//             <div class="col-md-9">
//             <h3 class="text1">ABOUT</h3>
//               <div class="card tt">
//                       <a href="#"><img src="8.jpg" alt="Card image cap" height="250px;"/> </a>  
//                       <p class="tt1"> 
//                         Thầy Hoàng CLB Hanul Taekwondo
//                       </p>
//               </div>
//               <div class="card tt">
//                       <a href="#"><img src="9.jpg" alt="Card image cap" height="250px;"/> </a>  
//                       <p class="tt1"> 
//                         Thầy Hoàng CLB Hanul Taekwondo
//                       </p>
//               </div>
//               <div class="card tt">
//                       <a href="#"><img src="10.jpg" alt="Card image cap" height="250px;"/> </a>  
//                       <p class="tt1"> 
//                         Thầy Hoàng CLB Hanul Taekwondo
//                       </p>
//               </div>
//             </div>
//             <div class="col-md-3">
//                     <h3 class="text1">NEWS</h3>
//                     <a href="#" class="neww"><img class="new1" src="trang.jpg" alt="Card image cap"/> Cuộc thi đai trắng<br/></a>
//                     <a href="#" class="neww"><img class="new1" src="vang.jpg" alt="Card image cap"/> Cuộc thi đai vàng<br/></a>
//                     <a href="#" class="neww"><img class="new1" src="xanh.jpg" alt="Card image cap"/> Cuộc thi vai xanh<br/></a>
//                     <a href="#" class="neww"><img class="new1" src="đỏ.jpg" alt="Card image cap"/> Cuộc thi vai đỏ<br/></a>
//                     <a href="#" class="neww"><img class="new1" src="den.jpg" alt="Card image cap"/> Cuộc thi vai đen</a>
//             </div>
//           </div>

//         </div>
//      </section>
//       {/*  */}
//       <h3 class="text1"> VIDEO </h3>
//       <div class="container">
//         <div class="row">
//             <div class="col-md-6">
//               <iframe src="https://www.facebook.com/plugins/video.php?height=322&href=https%3A%2F%2Fwww.facebook.com%2F100006113465295%2Fvideos%2F2743616872518735%2F&show_text=false&width=560&t=0" width="500" height="300"scrolling="no" frameborder="0" allowfullscreen="true" allow="autoplay; clipboard-write; encrypted-media; picture-in-picture; web-share" allowFullScreen="true"/>
//             </div>
//             <div class="col-md-6">
//               <iframe src="https://www.facebook.com/plugins/video.php?height=314&href=https%3A%2F%2Fwww.facebook.com%2F100006113465295%2Fvideos%2F2743616055852150%2F&show_text=false&width=560&t=0" width="500" height="300"scrolling="no" frameborder="0" allowfullscreen="true" allow="autoplay; clipboard-write; encrypted-media; picture-in-picture; web-share" allowFullScreen="true" />
//           </div>
//       </div>
//       </div>
// {/* footer */}
//       <div class="footer-text bg-dark">
//       <ul>
//             <li>
//                 <i class="fab fa-facebook-square"></i>
//             </li>
//             <li>
//                 <i class="fab fa-twitter-square"></i>
//             </li>
//             <li>
//                 <i class="fab fa-youtube"></i>
//             </li>
//             <li>
//                 <i class="fab fa-linkedin"></i>
//             </li>
//       </ul>
//           © 2021 Copyright:  <a href="#">ThuHiep.com</a>
//       </div>

//       </header>
//   </div>
//   );
// }

// export default App;
import React from 'react'
import './App.css';
import Header from './components/Header'
import Detail from './components/Detail'
import About from './components/About'
import Lienlac from './components/Lienlac'
import Video from './components/Video'
import Footer from './components/Footer'
import Teams from './components/Teams';
const LayoutMaster =()=>{
   return(
     <div>
          <Header/>
          <Detail/>
          <Teams/>
          <About/>
          <Lienlac/>
          <Video/>
          <Footer/>
     </div>
   )
}
export default React.memo(LayoutMaster)

